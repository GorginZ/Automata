...
The rules can be described algebraically or in boolean form. I have made a IRules interface which returns a binary like state for the next cell in the generation based on the boolean expression of the rule and have been adding unit tests for each rule and then writing the rule as a new class that implements IRules.
